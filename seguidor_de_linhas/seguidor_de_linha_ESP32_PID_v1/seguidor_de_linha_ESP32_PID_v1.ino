...
void loop() {
  ...
  velocidade = map(sliderValue.toInt(), 0, 255, 0, 255);
  ...
  // Define a direção dos motores usando a saída do PID
  analogWrite(motor1_A, veldir);
  analogWrite(motor1_B, 0);
  analogWrite(motor2_A, velesq);
  analogWrite(motor2_B, 0);

  delay(100); // Adicione um pequeno atraso para evitar leituras muito rápidas
